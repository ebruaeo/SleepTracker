...
fun formatNights(nights: List<SleepNight>, resources: Resources): Spanned {
    ...
    sb.apply {
        ...
        nights.forEach {
            ...
            if (it.endTimeMilii != it.startTimeMilli) {
                append(resources.getString(R.string.end_time))
                append("\t${convertLongToDateString(it.endTimeMilii)}<br>")
                append(resources.getString(R.string.quality))
                append("\t${convertNumericQualityToString(it.sleepQuality, resources)}<br>")
                append(resources.getString(R.string.hours_slept))
                // Hours
                append("\t ${it.endTimeMilii.minus(it.startTimeMilli) / 1000 / 60 / 60}:")
                // Minutes
                append("${it.endTimeMilii.minus(it.startTimeMilli) / 1000 / 60}:")
                // Seconds
                append("${it.endTimeMilii.minus(it.startTimeMilli) / 1000}<br><br>")
            }
        }
    }
